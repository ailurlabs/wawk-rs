...
//! ## Plugin Support
//!
//! Plugins are loaded on the JS side and bridged via `JsPluginBridge`.
//! When AWK calls an unknown function (e.g. `sha256("hello")`), the evaluator
//! routes it through the bridge → JS → plugin's `process_data` → result back.
//!
//! ```js
//! import init, { exec_awk, register_plugin } from './wawk.js';
//! await init();
//!
//! // Load a plugin
//! const hashWasm = await fetch('wawk-hello.wasm').then(r => r.arrayBuffer());
//! register_plugin('hash', new Uint8Array(hashWasm));
//!
//! // Use plugin functions from AWK
//! const result = exec_awk('{ print sha256($0) }', 'hello');
...
use wawk_core::traits::StubIncludeResolver;
...
// ============================================================================
// JS Plugin Dispatch — imported from the JS host
// ============================================================================

#[cfg(target_arch = "wasm32")]
#[wasm_bindgen]
extern "C" {
    /// JS-side plugin dispatch function.
    ///
    /// Called when AWK invokes a plugin function.
    /// - `name`: function name (e.g. "sha256")
    /// - `args`: NUL-separated arguments (e.g. "hello" or "key\0data")
    ///
    /// Returns the result string, or a string starting with "ERROR:" on failure.
    ///
    /// The JS host must provide this via the module's import object.
    //  The default no-op is patched into the generated JS.
    #[wasm_bindgen(js_name = "__wawk_plugin_dispatch__")]
    fn js_plugin_dispatch(name: &str, args: &str) -> String;
}

// ============================================================================
// JsPluginBridge — implements AwkExternalFunction by calling JS
// ============================================================================

/// Bridge between wawk's evaluator and JS-loaded plugins.
///
/// Implements `AwkExternalFunction` so the evaluator can route unknown
/// function calls through JS to the appropriate plugin's `process_data`.
#[cfg(target_arch = "wasm32")]
struct JsPluginBridge;

#[cfg(target_arch = "wasm32")]
impl AwkExternalFunction for JsPluginBridge {
    fn call_external_str(&mut self, name: &str, args: &[String]) -> AwkResult<Option<String>> {
        let args_joined = args.join("\0");
        let result = js_plugin_dispatch(name, &args_joined);

        if result.is_empty() {
            // JS returned empty → no plugin handled this function
            return Ok(None);
        }
...
fn create_evaluator<'a>(
    ...
    #[allow(unused_mut)]
    let mut eval = wawk_core::eval::Evaluator::new(reader, writer, env, cmd);
    #[cfg(target_arch = "wasm32")]
    eval.set_external_function_handler(Box::new(JsPluginBridge));
    eval
}

// ============================================================================
// Plugin Registration (JS side)
// ============================================================================

/// JS-bridge compatibility stub for the browser/Node.js plugin registration pattern.
///
/// **This function is a no-op on the Rust side.** The actual plugin instantiation
/// and registration happens entirely in JavaScript via `initPluginBridge()`.
///
/// # Why does this exist?
///
/// This stub exists so that wasm-bindgen generates the correct import/export
/// wiring at the JS<->Rust boundary. When JS calls `register_plugin(name, wasmBytes)`,
/// the JS-side plugin manager processes the wasm bytes, instantiates the plugin,
/// and sets up the dispatch bridge. This Rust function simply returns `true` to
/// acknowledge the call.
///
/// This function is **not used** in the WASM component model path (WIT plugins).
/// It exists solely for backwards compatibility with the wasm-bindgen-based
/// browser/Node.js plugin registration pattern.
///
/// # Arguments
/// * `name` - Plugin identifier (e.g. "hash")
/// * `_wasm_bytes` - The plugin's .wasm binary (unused on Rust side; processed by JS)
///
/// # Returns
/// Always returns `true`. The real registration happens in JS.
///
/// # JS Example
/// ```js
/// const wasmBytes = await fetch('wawk-hello.wasm').then(r => r.arrayBuffer());
/// register_plugin('hash', new Uint8Array(wasmBytes));
/// // -> JS-side initPluginBridge() handles instantiation
/// ```
#[wasm_bindgen]
pub fn register_plugin(name: &str, _wasm_bytes: &[u8]) -> bool {
    let _ = name;
    true
}
...
fn exec_awk_inner(script: &str, input: &str) -> Result<String, String> {
    let program = wawk_core::parser::parse(script).map_err(|e| e.to_string())?;
...
fn exec_awk_multi_inner(scripts: &[String], input: &str) -> Result<String, String> {
    let combined: String = scripts
        ...
        .collect::<Vec<&str>>()
        .join("\n");
    let program = wawk_core::parser::parse(&combined).map_err(|e| e.to_string())?;
...
impl HostReader {
    fn new(files_json: &str) -> Self {
        ...
        Self {
            ...
            filename_dirty: !file_order.is_empty(),
...
pub fn exec_awk_with_files(
    ...
    let expanded = match preprocessor::preprocess(script, &StubIncludeResolver) {
        Ok(s) => s,
...
mod tests {
    ...
    #[test]
    fn test_register_plugin_returns_true() {
        assert!(register_plugin("test", &[0, 1, 2, 3]));
    }

    #[test]
    fn test_register_plugin_empty_bytes() {
        assert!(register_plugin("empty", &[]));
    }

    #[test]
    fn test_register_plugin_empty_name() {
        assert!(register_plugin("", &[1, 2, 3]));
    }
    ...
    // ------------------------------------------------------------------------
    // JSON parsing: parse_files_json
    // ------------------------------------------------------------------------

    #[test]
    fn test_parse_files_json_empty_object() {
        let result = parse_files_json("{}");
        assert!(result.is_empty());
    }

    #[test]
    fn test_parse_files_json_empty_string() {
        let result = parse_files_json("");
        assert!(result.is_empty());
    }

    #[test]
    fn test_parse_files_json_single_file() {
        let result = parse_files_json("{\"data.txt\": \"line1\\nline2\"}");
        assert_eq!(result.len(), 1);
        let lines = result.get("data.txt").unwrap();
        assert_eq!(lines, &["line1", "line2"]);
    }

    #[test]
    fn test_parse_files_json_multiple_files() {
        let result = parse_files_json(
            "{\"a.txt\": \"hello\", \"b.txt\": \"world\"}"
        );
        assert_eq!(result.len(), 2);
        assert_eq!(result.get("a.txt").unwrap(), &["hello"]);
        assert_eq!(result.get("b.txt").unwrap(), &["world"]);
    }

    #[test]
    fn test_parse_files_json_escape_sequences() {
        // \n in JSON value becomes a real newline after parsing
        let result = parse_files_json("{\"f\": \"hello\\nworld\"}");
        let lines = result.get("f").unwrap();
