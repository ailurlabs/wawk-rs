...
mod tests {
    ...
    fn test_capability_search() {
        ...
        let mut meta = make_meta("namespace_b", vec!["formula_eval"]);
        meta.capabilities = vec!["expression_eval".to_string(), "grid_operations".to_string()];
        loader.register(meta, noop_dispatch());

        let mut meta2 = make_meta("namespace_c", vec!["cel_eval"]);
        ...
        assert_eq!(grid_plugins[0].name, "namespace_b");
    }
    ...
    fn test_init_dependency_order() {
        ...
        let crypto_dispatch: DispatchFn = Arc::new(move |name, _| {
            if name == "__init__" {
                ...
                assert_eq!(order, 1, "plugin-a should init second");
        ...
        loader.register(
            make_meta_with_deps("plugin-a", vec!["func_a"], vec!["license-plugin"]),
            crypto_dispatch,
        );
        loader.register(make_meta("license-plugin", vec!["activate"]), lic_dispatch);

        let warnings = loader.finalize();
        assert!(warnings.is_empty());

        let active = loader.active_plugins();
        assert_eq!(active[0].name, "license-plugin");
        assert_eq!(active[1].name, "plugin-a");
    }
    ...
    fn test_init_failure_with_dependent() {
        ...
        loader.register(
            make_meta("license-plugin", vec!["activate"]),
        ...
        assert!(warnings[0].contains("license-plugin"));
        assert!(warnings[1].contains("plugin-a"));
        // plugin-a cascaded: depends on failed license-plugin
        assert_eq!(loader.len(), 0);
    }
    ...
    fn test_namespace_registry_populated_in_finalize() {
        let mut loader = PluginRegistry::new();
        loader.register(
            make_meta_with_ns("plugin-b", vec!["func_c", "func_d"], "namespace_b"),
            echo_dispatch("namespace_b"),
        );
        loader.register(
            make_meta_with_ns("plugin-a", vec!["func_b"], "namespace_a"),
            echo_dispatch("namespace_a"),
        );
        loader.finalize();

        let ns_reg = &loader.namespace_registry;
        assert!(ns_reg.has_namespace("namespace_b"));
        assert!(ns_reg.has_namespace("namespace_a"));
    ...
    fn test_qualified_dispatch_via_namespace() {
        ...
        // Qualified call: "formula.sum" should route to formula plugin's "func_c"
        let result = loader
            .dispatch("namespace_b.func_c", &["1".to_string(), "2".to_string()])
            .unwrap();
        assert_eq!(result, Some("namespace_b:func_c(1,2)".to_string()));

        // Qualified call: "crypto.sha256" should route to crypto plugin's "func_b"
        let result = loader
            .dispatch("namespace_a.func_b", &["hello".to_string()])
            .unwrap();
        assert_eq!(result, Some("namespace_a:func_b(hello)".to_string()));
    }
    ...
    fn test_qualified_dispatch_unknown_namespace() {
        let mut loader = PluginRegistry::new();
        loader.register(
            make_meta_with_ns("plugin-b", vec!["func_c"], "namespace_b"),
            echo_dispatch("namespace_b"),
        );
        loader.finalize();
    ...
    fn test_default_namespace_dispatch() {
        ...
        loader.set_default_namespace("namespace_b");

        // Unqualified call should resolve via default namespace
        // Note: "func_c" is in fn_index, so it dispatches directly.
        ...
        let result = loader.dispatch("func_c", &[]).unwrap();
        assert_eq!(result, Some("namespace_b:func_c".to_string()));
    }
    ...
    fn test_unqualified_dispatch_still_works_with_namespaces() {
        ...
        let result = loader.dispatch("func_c", &["1".to_string()]).unwrap();
        assert_eq!(result, Some("namespace_b:func_c(1)".to_string()));
    }

    // ── T1-T6: Namespace/Plugin Unification Integration Tests ─────────

    /// T1: Two plugins with same function name coexist without collision.
    /// plugin-b and plugin-c both export `evaluate()`. They coexist
    /// because each has its own namespace.
    #[test]
    fn test_t1_same_function_name_coexistence() {
        let mut loader = PluginRegistry::new();
        loader.register(
            make_meta_with_ns("plugin-b", vec!["func_d", "func_c"], "namespace_b"),
            echo_dispatch("namespace_b"),
        );
        loader.register(
            make_meta_with_ns("plugin-c", vec!["func_d", "func_e"], "namespace_c"),
            echo_dispatch("namespace_c"),
        );
        let warnings = loader.finalize();
        assert!(warnings.is_empty(), "unexpected warnings: {:?}", warnings);
        assert_eq!(loader.len(), 2);

        // Both plugins are active and have "func_d" — no collision
        // because they are in different namespaces
        let formula_result = loader.dispatch("namespace_b.func_d", &["1+1".to_string()]).unwrap();
        assert_eq!(formula_result, Some("namespace_b:func_d(1+1)".to_string()));

        let cel_result = loader.dispatch("namespace_c.func_d", &["x > 5".to_string()]).unwrap();
        assert_eq!(cel_result, Some("namespace_c:func_d(x > 5)".to_string()));
    }
    ...
    fn test_t2_qualified_calls_route_via_namespace_registry() {
        ...
        loader.register(
            make_meta_with_ns("plugin-c", vec!["func_e", "func_d"], "namespace_c"),
            echo_dispatch("namespace_c"),
        );
        loader.finalize();
        ...
        let r1 = loader.dispatch_qualified("namespace_b", "func_c", &["1".to_string(), "2".to_string()]).unwrap();
        assert_eq!(r1, Some("namespace_b:func_c(1,2)".to_string()));

        let r2 = loader.dispatch_qualified("namespace_c", "func_e", &["x>0".to_string()]).unwrap();
        assert_eq!(r2, Some("namespace_c:func_e(x>0)".to_string()));

        // Cross-namespace: formula.evaluate vs cel.evaluate
        let r3 = loader.dispatch_qualified("namespace_b", "func_d", &["A1+B1".to_string()]).unwrap();
        assert_eq!(r3, Some("namespace_b:func_d(A1+B1)".to_string()));

        let r4 = loader.dispatch_qualified("namespace_c", "func_d", &["x>5".to_string()]).unwrap();
        assert_eq!(r4, Some("namespace_c:func_d(x>5)".to_string()));
    ...
    /// When "namespace_b" is the default namespace, unqualified sum() goes to formula.
    #[test]
    fn test_t3_unqualified_calls_route_to_default_namespace() {
        ...
        // Set default namespace to "namespace_b" (as @plugin "namespace_b" would do)
        loader.set_default_namespace("namespace_b");

        // has_namespace should confirm both are registered
        assert!(loader.has_namespace("namespace_b"));
        assert!(loader.has_namespace("namespace_c"));

        // Unqualified "func_c" should route to formula via fn_index
        let r1 = loader.dispatch("func_c", &["1".to_string()]).unwrap();
        assert_eq!(r1, Some("namespace_b:func_c(1)".to_string()));
    }
    ...
    fn test_t4_backward_compatibility_prefixed_functions() {
        ...
        loader.register(
            make_meta("plugin-b", vec!["formula_sum", "formula_Date", "formula_evaluate"]),
            echo_dispatch("namespace_b"),
        );
        let warnings = loader.finalize();
        assert!(warnings.is_empty());
        ...
        assert_eq!(r1, Some("namespace_b:formula_sum(1,2)".to_string()));

        let r2 = loader.dispatch("formula_Date", &["2024".to_string()]).unwrap();
        assert_eq!(r2, Some("namespace_b:formula_Date(2024)".to_string()));
    }
    ...
    fn test_t5_mixed_qualified_and_unqualified_calls() {
        ...
        // Set default namespace (as @plugin "namespace_b" would)
        loader.set_default_namespace("namespace_b");
        ...
        let r3 = loader.dispatch_qualified("namespace_b", "func_d", &["A1".to_string()]).unwrap();
        assert_eq!(r3, Some("namespace_b:func_d(A1)".to_string()));

        // Another unqualified call → still default namespace
        let r4 = loader.dispatch("func_d", &["B2".to_string()]).unwrap();
        // "func_d" is in fn_index for both plugins, but fn_index takes first registered
        // Since formula was registered first, it gets "func_d"
    ...
    /// Simulates switching from @plugin "namespace_b" to @plugin "namespace_c".
    #[test]
    fn test_edge_plugin_switch_updates_default_namespace() {
        ...
        loader.register(
            make_meta_with_ns("plugin-c", vec!["func_d"], "namespace_c"),
            echo_dispatch("namespace_c"),
        );
        loader.finalize();

        // First: default is "namespace_b"
        loader.set_default_namespace("namespace_b");
        assert_eq!(loader.namespace_registry.default_namespace(), Some("namespace_b"));

        // Switch: now default is "namespace_c" (as @plugin "namespace_c" mid-script would do)
        loader.set_default_namespace("namespace_c");
        assert_eq!(loader.namespace_registry.default_namespace(), Some("namespace_c"));
