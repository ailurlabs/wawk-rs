...
impl<'a> Evaluator<'a> {
    ...
    /// Dispatches directly to the external handler with the namespaced name.
    fn eval_qualified_func_call(
        ...
        // Build the qualified name: namespace.function
        ...
        if self.external_fn.is_some() {
            ...
            let str_result = handler.dispatch(&qualified_name, &str_args);
            match str_result {
                Ok(Some(result_str)) => {
                    if result_str.starts_with("ERROR:") {
                        self.errno = result_str.clone();
            ...
            // Also try with just the function name (plugin may register without prefix)
