...
fn benchmark_numeric_aggregation() {
    ...
    assert!(
        avg_ms < 200.0,
        "Numeric aggregation too slow: {:.2}ms",
        avg_ms
    );
}
...
fn benchmark_pattern_filtering() {
    ...
    assert!(
        avg_ms < 150.0,
        "Pattern filtering too slow: {:.2}ms",
        avg_ms
    );
}
...
fn benchmark_propertytree_native_access() {
    ...
    println!(
        "PropertyTree-native access (1k records): {:.2}ms avg",
        avg_ms
    );

    assert!(
        avg_ms < 100.0,
        "PropertyTree-native access too slow: {:.2}ms",
        avg_ms
    );
}
...
fn benchmark_property_tree_conversion() {
    ...
    assert!(
        avg_ms < 150.0,
        "PropertyTree conversion too slow: {:.2}ms",
        avg_ms
    );
}
